...
import {
  ArrowRight, ScanLine, Store, CloudSun, Sprout,
    ...
    <>
      ...
      <Section className="!pt-0">
        <div className="flex items-end justify-between gap-6">
          ...
          />
          <Link to="/agriculture-specialists" className="hidden text-sm font-semibold text-brand hover:underline sm:inline">View all →</Link>
        </div>
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <Link to="/agriculture-specialists" key={s.name} className="card-soft card-soft-hover overflow-hidden group">
              <div className="p-6">
                <div className="flex items-center gap-4">
                  <img src={s.img} alt={s.name} className="h-16 w-16 rounded-full object-cover" />
                  <div>
                    <h3 className="font-semibold">{s.name}</h3>
                    <p className="text-sm text-muted-foreground">{s.qualification}</p>
                    <div className="mt-1 flex flex-wrap gap-1">
                      {s.specialization.slice(0, 2).map((spec) => (
                        <span key={spec} className="text-[11px] rounded-full bg-accent px-2 py-0.5 text-accent-foreground">{spec}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 text-xs text-muted-foreground">{s.experience} experience</div>
              </div>
            </Link>
          ))}
        </div>
